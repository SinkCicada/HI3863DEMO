...
- `48_wifi_aircon_server`：空调红外文本控制
...
- `GPIO7`：DHT11
- `GPIO6`：门控舵机
- `GPIO0/1`：红外模块 TTL 串口（GPIO0->模块 RX，GPIO1<-模块 TX）
...
[hub] enabled: 45 temp/humi + 47 door servo + 48 aircon; 44 LED removed
[hub] server listening on 192.168.1.62:8000
[hub] protocol: door=binary32, temp=text command TEMP
[hub-ac] enabled, uart_baud=115200
```
...
建议把 DHT11 只作为“采集层”，温湿度阈值判断和是否触发空调放在软件策略层，而不是写死在底层驱动里。这样后续要改阈值、加模式、做手动/自动切换时，不需要再动硬件采集代码。
