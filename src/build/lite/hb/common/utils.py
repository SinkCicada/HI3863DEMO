...
def remove_path(path):
    ...
    def _long_path(target):
        abs_target = os.path.abspath(target)
        if os.name != 'nt' or abs_target.startswith('\\\\?\\'):
            return abs_target
        if abs_target.startswith('\\\\'):
            return '\\\\?\\UNC\\' + abs_target[2:]
        return '\\\\?\\' + abs_target
    ...
    for _ in range(3):
        try:
            shutil.rmtree(long_path, onerror=_handle_remove_error)
            return
        except FileNotFoundError:
            return
...
def _get_subprocess_text_options():
    if os.name == 'nt':
...
def exec_command(cmd, log_path='out/build.log', **kwargs):
    ...
    text_options = _get_subprocess_text_options()
...
def check_output(cmd, **kwargs):
    try:
        ...
        ret = subprocess.check_output(cmd,
                                      ...
                                      encoding=text_options['encoding'],
                                      errors=text_options['errors'],
                                      **kwargs)
