...
output_bin = framework_partition.get(build_product, {}).get("output_bin", {})
analysis_bin = framework_partition.get(build_product, {}).get("analysis_bin", {})


def builder(target, source, env):
    if 'windows' in get_systype() and env.BoardConfig().get("name") != "Hi3861":
...
builder_env.Replace(
    BUILD_OUT_DIR=join("$PROJECT_DIR", "out", builder_env.subst("${FRAMEWORK_BUILD_TARGET}"))
)
