...
if board.get("name") in ['Hi3861']:
    output_bin = board.get("upload").get("framework_partition").get("ohos-sources").get("output_bin", {})
    analysis_bin = board.get("upload").get("framework_partition").get("ohos-sources").get("analysis_bin", {})


def message(target, source, env):  # pylint: disable=unused-argument
    if "windows" in get_systype():
        return "warning: can't build on windows"
    fullcmd = [env.subst("$PYTHONEXE"), "build.py", framework_settings.get("target"), '-b', env.GetBuildType()]
...
builder_env.Replace(BUILD_OUT_DIR=join("$PROJECT_DIR", "out", framework_settings.get("target")))
