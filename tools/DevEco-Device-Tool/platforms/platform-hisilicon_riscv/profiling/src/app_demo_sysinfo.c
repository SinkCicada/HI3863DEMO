...
static void get_task_info(TSK_INFO_S* ptask_info, char* task_info)
{
    hi_u32 len = 0;
    hi_u32 i = 0;
...
static void *app_heap_task(const char *arg)
{
    ...
    TSK_INFO_S* pptask_info = HI_NULL;
    char sys_info[256];
    hi_u32 len = 0;

    while(1)
    {
        memset_s(sys_info, sizeof(sys_info), 0, sizeof(sys_info));
        len = 0;
        ...
        len = sprintf_s(sys_info + len, 256-len, "+profiling\r\n");  //start flag
        
        len += sprintf_s(sys_info + len, 256-len, "rtc=%d\r\n", hi_get_milli_seconds());  //insert time info
        
        len += sprintf_s(sys_info + len, 256-len, "mem=%d,", (mem_inf.total + mem_inf.total_lmp)); //total   
        len += sprintf_s(sys_info + len, 256-len, "%d,", (mem_inf.used + mem_inf.used_lmp)); //used
        len += sprintf_s(sys_info + len, 256-len, "%d\r\n", mem_inf.peek_size); //peak size

        len += sprintf_s(sys_info + len, 256-len, "os_resource=");
        len += sprintf_s(sys_info + len, 256-len, "%d,", os_resource_stat.timer_usage); //timer usage
        len += sprintf_s(sys_info + len, 256-len, "%d,", os_resource_stat.task_usage); //task usage
        len += sprintf_s(sys_info + len, 256-len, "%d,", os_resource_stat.sem_usage); //sem usage
        len += sprintf_s(sys_info + len, 256-len, "%d,", os_resource_stat.queue_usage); //queue usage
        len += sprintf_s(sys_info + len, 256-len, "%d,", os_resource_stat.mux_usage); // mux usage
        len += sprintf_s(sys_info + len, 256-len, "%d\r\n", os_resource_stat.event_usage); //event usage

        printf("%s\r\n", sys_info);

        pptask_info = (TSK_INFO_S*)hi_malloc(HI_MOD_ID_SAL_DFX, sizeof(TSK_INFO_S));
        if (pptask_info != HI_NULL){
            get_task_info(pptask_info, sys_info);
            hi_free(HI_MOD_ID_SAL_DFX, pptask_info);
        }
