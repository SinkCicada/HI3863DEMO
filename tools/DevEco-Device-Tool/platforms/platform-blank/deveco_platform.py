...
class BlankPlatform(PlatformBase):
    ...
    def configure_default_packages(self, options, targets):
        arch = options.get("debug_device", "Cortex-M3").split('|').pop()
        debug_packages = {
            "Cortex": {"gdb": ["arm_noneeabi_gcc"]},
            "RISCV": {"gdb": ["gcc_riscv32"]}
        }
        ...
        if debug_targets & set(targets):
            ...
            if arch.startswith('Cortex'):
                required_packages.update(debug_packages.get('Cortex', {}).get(debug_client, []))
            elif arch.startswith('RISC'):
                required_packages.update(debug_packages.get('RISCV', {}).get(debug_client, []))
    ...
    def _set_debug_task(self, tasks, board_id, env):
        ...
        if isinstance(board_config, BlankPlatform):
            try:
