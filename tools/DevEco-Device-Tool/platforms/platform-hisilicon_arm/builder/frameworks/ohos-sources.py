...
analysis_bin = board.get("frameworks").get("ohos-sources").get("analysis_bin", {})
rootfs_path = board.get("frameworks").get("ohos-sources").get("rootfs_path", {}).get("rootfs", "")
if board.get("name") in ['Hi3516DV300']:
    output_bin = board.get("upload").get("framework_partition") \
                        .get("ohos-sources").get("output_bin", {})
    analysis_bin = board.get("upload").get("framework_partition") \
                        .get("ohos-sources").get("analysis_bin", {})
    rootfs_path = board.get("upload").get("framework_partition") \
                        .get("ohos-sources").get("rootfs_path", {}).get("rootfs", "")
...
builder_env.Replace(ANALYSIS_ROOTFS_DIR = rootfs_path)
