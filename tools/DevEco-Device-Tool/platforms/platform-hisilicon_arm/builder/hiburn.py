...
def process_partitions(check_partition_file, parts, env):
    partitions = []
    for _, part in parts.items():
        if check_partition_file and not env.subst(part['partition_bin']):
            raise Errors.BuildError(
                errstr=f"Binary for partition `{part['partition_type']}` not found."
        ...
        if check_partition_file and not os.path.isfile(part["partition_bin"]):
            raise Errors.BuildErro(
                errstr=f"Binary for partition `{part['partition_type']}` not found: {part['partition_bin']}")
...
def HiBurnRunCmd(environ, target, source, env, action):  # pylint: disable=unused-argument
    ...
    partitions = process_partitions(check_partition_file, parts, env)
    ...
    if any(missed_options):
        click.secho(
            ...
            f" {['upload_' + k for k in missed_options]}",
            fg="red"
        )
        return -1
