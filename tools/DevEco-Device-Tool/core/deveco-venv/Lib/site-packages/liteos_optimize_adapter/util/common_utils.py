...
class CommonUtils:
    ...
    def get_prefix(path):
        ...
        if not exists(path) or not isdir(path):
            click.echo(f'compiler path not find or not a directory: {path}')
            raise FileExistsError()
        cp_files = listdir(path)

        win_sign = '-gcc.exe'
        linux_sign = '-gcc'
        llvm_win_sign = 'clang.exe'
        llvm_linux_sign = 'clang'
        has_compiler = False

        for file_name in cp_files:
            if file_name.endswith(win_sign):
                data['prefix'] = file_name[0: file_name.rindex(win_sign)]
                data['environment'] = 'windows'
                has_compiler = True
                break
            elif file_name.endswith(linux_sign):
                data['prefix'] = file_name[0: file_name.rindex(linux_sign)]
                data['environment'] = 'linux'
                has_compiler = True
                break
            elif file_name == llvm_win_sign:
                data['prefix'] = 'llvm'
                data['environment'] = 'windows'
                has_compiler = True
                break
            elif file_name == llvm_linux_sign:
                data['prefix'] = 'llvm'
                data['environment'] = 'linux'
                has_compiler = True
                break
        if not has_compiler:
            click.echo(f'Incorrect compiler path: {path}')
            raise Exception()
    ...
    @staticmethod
    def get_obj_dump_info(obj_dump_path, arg, elf_file_path, is_return_arr=True):
        result = []
