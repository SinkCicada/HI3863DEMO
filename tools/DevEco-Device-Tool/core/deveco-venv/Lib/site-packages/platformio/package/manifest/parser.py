...
class ManifestParserFactory(object):
    ...
    def new(  # pylint: disable=redefined-builtin
        ...
        for _, cls in globals().items():
            is_base_class = inspect.isclass(cls) and issubclass(cls, BaseManifestParser)
            if is_base_class and cls != BaseManifestParser and cls.manifest_type == type:
...
class BaseManifestParser(object):
    def __init__(self, contents, remote_url=None, package_dir=None):
        ...
        try:
            ...
            raise ManifestParserError("Could not parse manifest") from e
    ...
    def normalize_author(author):
        if not isinstance(author, dict):
            raise TypeError(f"{author} not a dict")
        if author.get("email"):
    ...
    def parse_examples_from_dir(package_dir):
        if not os.path.isdir(package_dir):
            raise FileNotFoundError("Not a directory: %s" % package_dir)
        ...
        allowed_exts = (".c", ".cc", ".cpp", ".h", ".hpp", ".asm", ".ASM", ".s", ".S", ".ino", ".pde", )
            ...
            if last_pio_project:
                if root.startswith(last_pio_project):
                    result[last_pio_project]["files"].extend(
                        [os.path.relpath(os.path.join(root, f), last_pio_project) for f in files]
                    )
                    continue
                last_pio_project = None
...
class LibraryJsonManifestParser(BaseManifestParser):
    ...
    def _str_to_list(value, sep=",", lowercase=True):
        ...
        if not isinstance(value, list):
            raise TypeError(f"{value} not a list")
        result = []
        for item in value:
            item = item.strip()
            if not item:
    ...
    def _parse_platforms(raw):
        if not isinstance(raw, list):
            raise TypeError(f"{raw} not a list")
        result = []
        # renamed platforms
        for item in raw:
            if item == "espressif":
    ...
    def _dependency(self, raw, i, dependency):
    ...
    def _parse_dependencies(raw):
        ...
        if isinstance(raw, list):
            for i, dependency in enumerate(raw):
                self._dependency(raw, i, dependency)
            return raw
...
class LibraryPropertiesManifestParser(BaseManifestParser):
    ...
    def _parse_repository(self, properties):
        github_url = read_url_json().get('github_url', '')
        if self.remote_url:
            ...
            if "github" in url_attrs.netloc:
                return dict(
                    type="git",
                    url=github_url + "/".join(repo_path_tokens[:2]),
                )
        ...
        if properties.get("url", "").startswith(github_url):
