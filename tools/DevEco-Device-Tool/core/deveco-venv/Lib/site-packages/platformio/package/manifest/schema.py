...
if MARSHMALLOW_2:
    ...
    class CompatSchema(Schema):
        ...
        def handle_error(self, error, data, **_):  # pylint: disable=arguments-differ, no-self-use
...
class ManifestSchema(BaseSchema):
    ...
    def validate_version(self, value):  # pylint: disable=no-self-use
        try:
            ...
            semver_url = read_url_json().get('semver_url', '')
            raise ValidationError(f"Invalid semantic versioning format, see {semver_url}") from e
    ...
    def validate_license(self, value):
        try:
            ...
            raise ValidationError("Could not load SPDX licenses for validation") from e
        for item in spdx.get("licenses", []):
            if item.get("licenseId") == value:
                return
        spdx_licenses_url = read_url_json().get('spdx_licenses_url', '')
        raise ValidationError(f"Invalid SPDX license identifier. See valid identifiers at {spdx_licenses_url}")

    @staticmethod
    @memoized(expire="1h")
    def load_spdx_licenses():
        githubusercontent_licenses_url = read_url_json().get('githubusercontent_licenses_url', '')
        r = requests.get(githubusercontent_licenses_url)
        r.raise_for_status()
        return r.json()
