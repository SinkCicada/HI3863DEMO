...
class LibraryManager(BasePkgManager):
    ...
    def _install_from_piorepo(self, name, requirements):
        if not name.startswith("id="):
            raise ValueError(f"{name} not startswith \"id=\" ")
        ...
        if not dl_data:
            raise ValueError(f"Invalid value: {dl_data}")
    ...
    def search_lib_id(  # pylint: disable=too-many-branches
        ...
        if not isinstance(filters, dict):
            raise TypeError(f"{filters} not a dict")
        if "name" not in filters:
            raise ValueError(f"\"name\" not in the {filters}")
        ...
        # looking in DevEco Library Registry
        if not silent:
            click.echo(
        ...
        lib_info = self._handle_result_total(query, silent, interactive, filters)
        ...
        if not silent:
            ...
            click.echo(
                "Found: %s"
                % click.style(
                    f"{platformio_url}/lib/show/{id}/{name}".format(**lib_info),
                    fg="blue",
                )
            )
        return int(lib_info["id"])

    def _handle_result_total(self, query, silent, interactive, filters):
        lib_info = None
        ...
        if result["total"] == 1:
            ...
            if silent and not interactive:
                ...
                lib_info = self._handle_interactive(interactive, result, lib_info)

        return lib_info

    def _handle_interactive(self, interactive, result, lib_info):
        if not interactive:
            click.secho(
    ...
    def _install(self, name, requirements):
        raise Exception('This method should not be called for LibraryManager')
    ...
    def install(self, name, **kwargs):
        ...
        if not _url:
            name = "id=%d" % self.search_lib_id(
                {"name": _name, "requirements": _requirements}, silent=silent, interactive=interactive,
            )
            requirements = _requirements
        pkg_dir = BasePkgManager.install(
            self, name, requirements, silent=silent, after_update=after_update, force=force,
        )
        ...
        self.handle_dep_filters(builtin_lib_storages, manifest, params)

        return pkg_dir

    def handle_dep_filters(self, builtin_lib_storages, manifest, params):
        for filters in manifest["dependencies"]:
            ...
            if any(s in filters.get("version", "") for s in ("\\", "/")):
                self.install(
                    ...
                    silent=params.get('silent'), after_update=params.get('after_update'),
                    interactive=params.get('interactive'), force=params.get('force'),
                )
            else:
                try:
                    lib_id = self.search_lib_id(filters, params.get('silent'), params.get('interactive'))
                except exception.LibNotFound as e:
                    if builtin_lib_storages is None:
                        builtin_lib_storages = get_builtin_libs()
                    if not params.get('silent') or is_builtin_lib(
                ...
                if filters.get("version"):
                    self.install(
                        ...
                        after_update=params.get('after_update'),
