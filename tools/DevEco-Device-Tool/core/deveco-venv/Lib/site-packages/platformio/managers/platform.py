...
# pylint: disable=too-many-public-methods, too-many-instance-attributes, too-many-lines, too-many-locals
...
from platformio.helpers import get_executable_path, get_upload_tool_path
import platformio.feature
from platformio.util import read_url_json
import platformio.logger
from platformio.helpers import get_compile_log_config


log_config =  get_compile_log_config()
build_folder = os.path.join(os.path.join(os.path.expanduser('~'),log_config.get('folder')))

if not os.path.exists(build_folder):
    os.makedirs(build_folder)
log_config['folder'] = build_folder


logger = platformio.logger.Logger(**log_config, level='info')
...
class PlatformManager(BasePkgManager):
    def __init__(self, package_dir=None, repositories=None):
        if not repositories:
            dl_url = read_url_json()
            dl_platform_url = dl_url.get('dl_platform_url', '')
            dl_platformio_s_url = dl_url.get('dl_platformio_s_url', '')
            dl_platformio_url = dl_url.get('dl_platformio_url', '')
            repositories = [
                dl_platform_url,
                dl_platformio_s_url if app.get_setting("strict_ssl") else dl_platformio_url
            ]
        self.config = ProjectConfig.get_instance()
        super().__init__(
            package_dir or self.config.get_optional_dir("platforms"), repositories
        )
    ...
    def _install(self, name, requirements):
        raise Exception('This method should not be called for PlatformManager')

    def install(self, name, **kwargs):
        requirements = kwargs.get("requirements", None)
        with_packages = kwargs.get("with_packages", None)
        without_packages = kwargs.get("without_packages", None)
        skip_default_package = kwargs.get("skip_default_package", False)
        with_all_packages = kwargs.get("with_all_packages", False)
        after_update = kwargs.get("after_update", False)
    ...
    def get_all_boards(self):
        ...
        try:
            ...
            pass
        except exception.InternetIsOffline:
            pass
    ...
    def get_installed(self):
        items = super().get_installed()
        filtered = [p for p in items if platformio.feature.get(f'platforms.{p["name"]}') is not False]
        return filtered
...
class PlatformFactory(object):
    ...
    def get_clsname(name):
        ...
        return "%sPlatform" % ("".join([n.capitalize() for n in name]))
    ...
    def load_module(name, path):
        try:
            ...
            raise exception.UnknownPlatform(name) from e
    ...
    def newPlatform(cls, name, requirements=None):
        ...
        platform_cls = None
        if isfile(join(platform_dir, "deveco_platform.py")):
            platform_cls = getattr(
                cls.load_module(name, join(platform_dir, "deveco_platform.py")),
        ...
        if not isinstance(_instance, PlatformBase):
            raise TypeError(f"{_instance} is not the type of PlatformBase")
        return _instance
...
class PlatformPackagesMixin(object):
    ...
    def get_upload_tool(self, name):
        tool_dir = join(get_upload_tool_path(), name)
        return tool_dir if isdir(tool_dir) else None
...
class PlatformRunMixin(object):
    ...
    def run(self, variables, targets, silent, **kwargs):
        ...
        if not isinstance(variables, dict):
            raise TypeError(f"{variables} not a dict")
        if not isinstance(targets, list):
            raise TypeError(f"{targets} not a list")
        ...
        result = self._run_scons(variables, targets, jobs, list_files, custom_tool_path)
        if "returncode" not in result:
            raise ValueError("returncode not in the result")

        return result

    def _run_scons(self, variables, targets, jobs, list_files, custom_tool_path):
        self.targets = targets
        args = [
            proc.get_pythonexe_path(),
            join(os.path.dirname(proc.get_pythonexe_path()),
                "scons" if sys.platform.startswith("linux") else "scons.exe"),
            "-Q", "--warn=no-no-parallel-support",
            "--jobs", str(jobs),
            "--sconstruct", join(fs.get_source_dir(), "builder", "main.py"),
        ]
        args.append("CLIENTPORT=%s" % (self.client_port if self.client_port else '0'))
        args.append("PIOVERBOSE=%d" % (1 if self.verbose else 0))
        args.append("PIOLISTFILES=%d" % (1 if list_files else 0))
        if custom_tool_path:
            args.append(f"PIOCUSTOMTOOL={custom_tool_path}")
        ...
        if targets and set(targets) & {"menuconfig", "patch_kernel", "run"}:
        ...
        if click._compat.isatty(sys.stdout):
            ...
            for target in targets:
                if target == "buildprog":
                    logger.info(self._log_target_banner(target))
            ...
            result = proc.exec_command(
                ...
                logger=logger if "buildprog" in args else None,
                log_path=os.path.normpath(os.path.join(build_folder,log_config.get('file')))
            )
            return result
        result = proc.exec_command(
            args,
    ...
    @staticmethod
    def _log_target_banner(target):
        current_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        terminal_width, _ = shutil.get_terminal_size()
        banners = target + ':' + current_time
        label_width = len(click.unstyle(banners))
        half_line = '-' * int((terminal_width - label_width - 2) / 2)
        return "%s %s %s" % (half_line, banners, half_line)
    ...
    def _is_scons_error(self, line):
        scons_tasks = ["[upload]", "[stackAnalysis]", "[imageAnalysis]"]
        if any([f for f in scons_tasks if f in line]):
            return True
        return False 

    def _echo_line(self, line, level):
        if "buildprog" in self.targets:
            logger.info(line.rstrip('\n'))
        if line.startswith("scons: "):
            line = line[7:]
        if not (1 <= level <= 3):
            raise ValueError(f"Invalid value: {level} not in the valid range")
        if self.silent and (level < 2 or not line):
            return
        fg = (None, "yellow", "red")[level - 1]
        if level == 1 and "is up to date" in line:
            fg = "green"
        if level > 2 and self._is_scons_error(line):
            logger.info(line.rstrip('\n'))
            return
...
class PlatformBase(PlatformPackagesMixin, PlatformRunMixin):
    ...
    def packages(self):
        ...
        for item in self._custom_packages or []:
            ...
            packages[name].update({
                ...
                "optional": False,
                "type": packages[name]['type'] if 'type' in packages[name] else 'tool'
            })
        return packages
    ...
    def get_settings_dir(self, board):
        settings = join(self.get_dir(), "settings", f"{board}.json")
        if isfile(settings):
            return settings
        return ''
    ...
    def get_boards(self, id_=None):
        def _append_board(board_id, manifest_path):
            ...
            if "platforms" in config and self.name not in config.get("platforms"):
                return
            feature_path = f'platforms.{self.name}.boards.'
            for name in platformio.feature.get_list(prefix=feature_path):
                if config.get("name") == name[len(feature_path):] and not platformio.feature.get(name):
                    return

            config.manifest["platform"] = self.name
            self._BOARDS_CACHE[board_id] = config

        def _id_none_handle(bdirs):
        ...
        def _id_notnone_handle(id_, bdirs):
        ...
        if id_ is None:
            _id_none_handle(bdirs)
        else:
            _id_notnone_handle(id_, bdirs)
            if id_ not in self._BOARDS_CACHE:
                raise exception.UnknownBoard(id_)
    ...
    def uninstall_python_packages(self):
        ...
        try:
            ...
            click.secho("Could not install Python packages", fg="red", err=True)
    ...
    @staticmethod
    def _generate_exclusive(group_list, env):
        return [f'{grp}-{env}' for grp in group_list]

    def tasks(self, board_id, project, env):  # pylint: disable=too-many-statements,too-many-branches
        def task(name, title, icon, cmd, exclusive=None):
            t = {'type': 'task', 'name': name, 'title': title, 'id': f'deveco: {name.lower()} - {env}',
                 'icon': icon, 'cmd': cmd, 'execution-type': 'process',
                 'exclusive': self._generate_exclusive(exclusive or [], env)}
            for k in [k for k, v in t.items() if v is None]:
                t.pop(k)
            return t

        def regular_operations():
            encoding = self.config.get(f'env:{env}', 'monitor_encoding', 'UTF-8')

            tasks.append(task(name='Clean', title='Clean output', icon='clean', exclusive=['build'],
                              cmd=[self._hos(), 'run', '--target', 'clean'] + arg_project + arg_env + arg_verbose))

            tasks.append(task(name='Build', title='Build target binary', icon='build', exclusive=['build'],
                              cmd=[self._hos(), 'run'] + arg_project + arg_env + arg_verbose))

            tasks.append(task(name='Rebuild', title='Rebuild target binary', icon='rebuild', exclusive=['build'],
                              cmd=[self._hos(), 'run', '--target', 'clean', '--target', 'buildprog']
                                  + arg_project + arg_env + arg_verbose))

            tasks.append(task(name='Upload', title='Upload to device', icon='burn', exclusive=['upload'],
                              cmd=[self._hos(), 'run', '--target', 'upload'] + arg_project + arg_env + arg_verbose))

            if board_config.get("build", {}).get("cpu", "") != "risc-v":
                tasks.append(task(name='Erase', title='Erase device', icon='warning', exclusive=['upload'],
                                  cmd=[self._hos(), 'run', '--target', 'erase'] + arg_project + arg_env + arg_verbose))

            tasks.append(task(name='Monitor', title='Start device monitor', icon='monitor',
                              cmd=[self._hos(), 'device', 'monitor', '--encoding', encoding] + arg_project + arg_env))

        def bootloader_operations():
            for target, cfg in board_config.get('uboot', {}).get('config', {}).items():
                tasks.append(task(name=f'Configure bootloader ({cfg["name"]})', title=None, icon='tool', exclusive=['bootloader'],
                                  cmd=[self._hos(), 'run', '--target', target] + arg_project + arg_env + arg_verbose))

        def partition_operations():
            board_partitions = board_config.get('upload', {}).get('partition_types', {})
            for partition in [x[len('partition:'):] for x in self.config.get(f'env:{env}', 'upload_partitions', [])]:
                partition_type = self.config.get(f'partition:{partition}', 'partition_type')
                non_erasable = board_partitions.get(partition_type, {}).get('non_erasable', False)
                arg_partition = ['--partition', f'partition:{partition}']

                partition_group = {'type': 'group',
                                   'name': partition,
                                   'title': f'Partition {partition}',
                                   'icon': 'partition',
                                   'items': []}

                partition_group['items'].append({'type': 'task',
                                                'name': 'Upload',
                                                'title': f'Upload {partition} to device',
                                                'id': f'deveco: upload - {env} - {partition}',
                                                'icon': 'burn',
                                                'cmd': [self._hos(), 'run', '--target', 'upload'
                                                        ] + arg_project + arg_env + arg_partition + arg_verbose,
                                                'exclusive': self._generate_exclusive(['upload'], env),
                                                'execution-type': 'process'})
                if not non_erasable:
                    partition_group['items'].append({'type': 'task',
                                                'name': 'Erase',
                                                'title': f'Erase {partition} on device',
                                                'id': f'deveco: erase - {env} - {partition}',
                                                'icon': 'warning',
                                                'cmd': [self._hos(), 'run', '--target', 'erase'
                                                        ] + arg_project + arg_env + arg_partition + arg_verbose,
                                                'exclusive': self._generate_exclusive(['upload'], env),
                                                'execution-type': 'process'})

                tasks.append(partition_group)
        ...
        def debug_operations():
            ...
            if isinstance(board_config, PlatformBoardConfig):
                try:
            ...
            elif debug_tool in ['openocd', 'openocd(HiSpark-Link/FT2232H Debugger)', 'openocd(HiSpark-Trace)']:
                run_debug_server['daemon'] = {
                    'starting-pattern': '^.*Debug\\sServer\\sConnected*',
        ...
        try:
            ...
            board_config = {}
        except exception.InvalidNetworkAccessException:
            board_config = {}

        # Regular operations
        regular_operations()

        # Configure bootloader operations
...
class PlatformBoardConfig(object):
    def __init__(self, manifest_path):
        ...
        if not isfile(manifest_path):
            raise FileNotFoundError("Not a file: %s" % manifest_path)
        self.manifest_path = manifest_path
        try:
            self._manifest = fs.load_json(manifest_path)
        except ValueError as e:
            raise exception.InvalidBoardManifest(manifest_path) from e
    ...
    def get_brief_data(self):
        return {
            ...
            "cpu": self._manifest.get("build", {}).get("cpu", "").upper(),
            ...
            "frameworks": [{**v, "name": k, "title": k.capitalize()}
                           for k, v in self._manifest.get("frameworks").items()],
            "debug": self.get_debug_data(),
            "vendor": self._manifest["vendor"],
            "url": self._manifest["url"],
            "upload": self._manifest["upload"],
            "monitor": self._manifest.get("monitor", {}),
            "uboot": [{"target": k, "title": v["name"]}
                      for k, v in self._manifest.get("uboot", {}).get("config", {}).items()],
            "memory": self._manifest.get("memory", {}),
            "options": self._manifest.get("options", []),
            "needSDK": self._manifest.get("needSDK", False),
            "supportedOs": self._manifest.get("supported_os", ['windows', 'linux']),
            "configOptions": self._manifest.get("config_options", {}),
        }

    def get_debug_data(self):
        if "debug" not in self._manifest:
            return None
        if "tools" not in self._manifest["debug"]:
            return {}
        tools = {}
        ...
        clients = {}
        for client in self._manifest["debug"]["clients"]:
            clients[client] = {}
        return {"tools": tools, "clients": clients}
