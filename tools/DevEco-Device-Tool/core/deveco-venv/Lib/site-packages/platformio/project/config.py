...
from platformio.compat import PY2, hashlib_encode_data, string_types
from platformio.project import exception
from platformio.project.options import ProjectOptions
from platformio import __version__, __ini_version__
...
; Version: {0}
; Core Version: {1}
""".format(__ini_version__, __version__)
...
class ProjectConfigBase(object):
    ...
    def read(self, path, parse_extra=True):
        ...
        try:
            ...
            raise exception.InvalidProjectConfError(path, 'config parser error') from e

        if not parse_extra:
            return

        # load extra configs
        for pattern in self.get("deveco", "extra_configs", []):
    ...
    def _maintain_renaimed_options(self):
        # legacy `lib_extra_dirs` in [deveco]
        if self._parser.has_section("deveco") and self._parser.has_option(
            "deveco", "lib_extra_dirs"
        ):
            ...
            self._parser.set(
                "env",
                "lib_extra_dirs",
                self._parser.get("deveco", "lib_extra_dirs"),
            )
            self._parser.remove_option("deveco", "lib_extra_dirs")
            self.warnings.append(
                "`lib_extra_dirs` configuration option is deprecated in "
                "section [deveco]! Please move it to global `env` section"
            )
    ...
    def options(self, section=None, env=None):
        result = []
        if not (section or env):
            raise ValueError("section and env cannot be all None ")
        if not section:
            section = "env:" + env
    ...
    def getraw(  # pylint: disable=too-many-branches
        ...
        value = self._handle_sysenvvar(option_meta, value)
    ...
    def _check_old_sysenvvar(self, option_meta):
        envvar_value = None
    ...
    def get(self, section, option, default=MISSING):
        ...
        try:
            ...
            raise exception.InvalidProjectConfError(self.path, 'config parser error') from e
    ...
    def default_envs(self):
        return self.get("deveco", "default_envs", [])
    ...
    def get_version(self, path=None):
        with open(path or self.path, 'r') as config_file:
            data = config_file.read()
            version_pattern = r'; Version: (.*)'
            info = re.findall(version_pattern, data)
            if not info:
                raise exception.ProjectError()
            return info[0]


class ProjectConfigDirsMixin(object):
    def get_optional_dir(self, name, exists=False):
        if not ProjectOptions.get("deveco.%s_dir" % name):
            raise ValueError("Unknown optional directory -> " + name)

        if name == "core":
            result = ProjectOptions["deveco.core_dir"].default
        else:
            result = self.get("deveco", name + "_dir")
...
class ProjectConfig(ProjectConfigBase, ProjectConfigDirsMixin):
    ...
    def update(self, data, clear=False):
        if not isinstance(data, list):
            raise TypeError(f"{data} is not list")
        if clear:
    ...
    def save(self, path=None):
        ...
        with os.fdopen(os.open(path or self.path, flags, 0o640), "w+") as fp:
