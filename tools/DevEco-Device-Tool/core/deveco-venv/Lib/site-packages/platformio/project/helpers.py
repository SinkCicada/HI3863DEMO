...
def is_platformio_project(project_dir=None):
    ...
    return isfile(join(project_dir, ".deveco", "deveco.ini"))
...
def get_project_core_dir():
    ...
    return ProjectConfig.get_instance(
        join(get_project_dir(), ".deveco", "deveco.ini")
...
def get_default_projects_dir():
    ...
    try:
        ...
        click.secho('get default projects dir error', fg='red')
    return join(docs_dir, "DevEco", "Projects")


def _collect_chunks(directory, check_suffixes):
    chunks = []
...
def compute_project_checksum(config):
    # rebuild when DevEco Core version changes
    ...
    for d in (
        ...
        if not isdir(d):
            continue
        chunks = _collect_chunks(d, check_suffixes)
        if not chunks:
            continue
...
def load_project_ide_data(project_dir, env_or_envs):
    ...
    if not env_or_envs:
        raise ValueError(f"Invalid value: {env_or_envs}")
    envs = env_or_envs
