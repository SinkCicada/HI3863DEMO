...
def load_json(file_path):
    try:
        ...
        raise exception.InvalidJSONFile(file_path) from e
...
def match_src_files(src_dir, src_filter=None, src_exts=None, followlinks=True):
    ...
    def _src_file_handle(item, items, src_dir, followlinks):
    ...
    for (action, pattern) in re.findall(r"(\+|\-)<([^>]+)>", src_filter):
        ...
        for item in glob(os.path.join(glob_escape(src_dir), pattern)):
            if os.path.isdir(item):
                _src_file_handle(item, items, src_dir, followlinks)
            else:
...
def rmtree(path):
    def _onerror(func, path, __):
        try:
            ...
            click.secho(
                "\nPlease manually remove the file `%s`" % (path),
                fg="red",
                err=True,
            )
