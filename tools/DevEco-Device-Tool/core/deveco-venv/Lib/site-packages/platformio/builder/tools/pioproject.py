...
def GetPartitionOptions(env, partition_name, as_dict=False):
    return env.GetProjectConfig().items(section=partition_name, as_dict=as_dict)
...
def generate(env):
    ...
    env.AddMethod(GetPartitionOptions)
