...
def get_build_type(env):
    ...
    if "ohos-sources" in framework:
        build_type = "debug"
    elif "hpm" in framework:
        build_type = "release"
    elif "hb" in framework:
        build_type = "debug"
    else:
        build_type = "release"
    return build_type
...
def GetBuildType(env):
    return (
        ...
        if (
            ...
            or env.GetProjectOption("build_type", get_build_type(env)) == "debug"
        )
        else "release"
    )
...
def ProcessProjectDeps(env):
    ...
    # extra build flags from `deveco.ini`
...
def ProcessUnFlags(env, flags):
    ...
    for key, _ in parsed.items():
        if key.endswith("FLAGS"):
            parsed[key].extend(all_flags)

    for key, unflags in parsed.items():
        for unflag in unflags:
            handle_unflags(env, key, unflag)


def handle_unflags(env, key, unflag):
...
def BuildFrameworks(env, frameworks):
    ...
    if "BOARD" not in env:
        sys.stderr.write(
            "Please specify `board` in `deveco.ini` to use "
            "with '%s' framework\n" % ", ".join(frameworks)
        )
        env.Exit(1)
    ...
    if frameworks == ["platformio"]:
        if board_frameworks:
            ...
            sys.stderr.write("Error: Please specify `board` in `deveco.ini`\n")
            env.Exit(1)
