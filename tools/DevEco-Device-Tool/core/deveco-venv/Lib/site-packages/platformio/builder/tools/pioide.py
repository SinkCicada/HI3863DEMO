...
def _get_svd_path(env):
    ...
    try:
        ...
        if not svd_path:
            raise ValueError(f"Invalid value: {svd_path}")
...
def _get_prog_path(env):
    prog_path = env.GetProjectOption("debug_elf")
    if prog_path and not os.path.isabs(prog_path):
        prog_path = os.path.join(env.subst("$PROJECT_DIR"), prog_path)
    if not prog_path:
        prog_path = env.subst("$PROG_PATH")
    return prog_path

def _get_live_watch_enable(env):
    live_watch_enable = env.GetProjectOption("debug_live_watch_enable")
    return live_watch_enable

def _get_hssclient_path(env):
    hssclient_path = env.GetProjectOption("debug_hssclient_path")
    if hssclient_path and not os.path.isabs(hssclient_path):
        hssclient_path = os.path.join(env.subst("$PROJECT_DIR"), hssclient_path)
    return hssclient_path

def _get_is_custom_prog_path(env):
    return bool(env.GetProjectOption("debug_elf"))
...
def DumpIDEData(env):
    ...
    data = {
        ...
        "prog_path": _get_prog_path(env),
        "live_watch_enable": _get_live_watch_enable(env),
        "hssclient_path": _get_hssclient_path(env),
        "build_out_dir": env.subst("$BUILD_OUT_DIR"),
        "is_custom_prog_path": _get_is_custom_prog_path(env),
        "flash_extra_images": [
