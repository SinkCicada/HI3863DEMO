...
class LibBuilderFactory(object):
    ...
    def new(env, path, verbose=int(ARGUMENTS.get("PIOVERBOSE", 0))):
        ...
        if not isinstance(obj, LibBuilderBase):
            raise TypeError(f"{obj} is not the type of {LibBuilderBase}")
        return obj
...
class LibBuilderBase(object):
    ...
    def _get_all_include_directories(self):
        # all include directories
        if not LibBuilderBase._INCLUDE_DIRS_CACHE:
            LibBuilderBase._INCLUDE_DIRS_CACHE = [
                self.env.Dir(d)
                for d in ProjectAsLibBuilder(self.envorigin, "$PROJECT_DIR").get_include_dirs()
            ]
            for lb in self.env.GetLibBuilders():
    ...
    def _get_found_includes(  # pylint: disable=too-many-branches
        ...
        self._get_all_include_directories()
        ...
        for path in search_files or []:
            ...
            try:
                if "+" not in self.lib_ldf_mode:
                    raise ValueError(f" + not in the {self.lib_ldf_mode}")
    ...
    def generate_map(self, inc, lib_inc_map):
        for lb in self.env.GetLibBuilders():
    ...
    def find_lib_inc_map(self, search_files):
        lib_inc_map = {}
        for inc in self._get_found_includes(search_files):
            lib_inc_map = self.generate_map(inc, lib_inc_map)
        return lib_inc_map
    ...
    def search_deps_recursive(self, search_files=None):
        ...
        if self.lib_ldf_mode == "off":
            return
        lib_inc_map = self.find_lib_inc_map(search_files)
...
class ArduinoLibBuilder(LibBuilderBase):
    ...
    def src_filter(self):
        ...
        if isdir(src_dir):
            ArduinoLibBuilder.handle_src_filter(src_dir)
    ...
    @staticmethod
    def handle_src_filter(src_dir):
        src_filter = LibBuilderBase.src_filter.fget()
    ...
    def _mbed_conf_append_macros(self, mbed_config_path, macros):
        ...
        with open(mbed_config_path) as fp:
            for line in fp.readlines():
                line = line.strip()
                if line == "#endif":
                    lines.append("// DevEco Library Dependency Finder (LDF)")
                    lines.extend(
                        [
        ...
        with os.fdopen(os.open(mbed_config_path, flags, 0o640), 'w') as fp:
            fp.write("\n".join(lines))
...
class PlatformIOLibBuilder(LibBuilderBase):
    ...
    def get_include_dirs(self):
        ...
        is_include = all([
            ...
            self._has_arduino_manifest(),
            not isdir(join(self.path, "src")),
...
class ProjectAsLibBuilder(LibBuilderBase):
        ...
        if "__test" in COMMAND_LINE_TARGETS:
            items.extend(
                [
                    ...
                    for item in self.env.MatchSourceFiles("$PROJECT_TEST_DIR", "$PIOTEST_SRC_FILTER")
                ]
            )
        return items
    ...
    def install_dependencies(self):
        ...
        for uri in not_found_uri:
            try:
                ...
                click.secho("Warning! internet is offline", fg="yellow")
    ...
    def process_dependencies(self):  # pylint: disable=too-many-branches
        for uri in self.dependencies:
            found = False
            self.handle_process_dep(found, uri)
            if found:
                continue
    ...
    def handle_process_dep(self, found, uri):
        for storage_dir in self.env.GetLibSourceDirs():
            if found:
                break
            lm = LibraryManager(storage_dir)
...
def GetLibBuilders(env):  # pylint: disable=too-many-branches
    ...
    for storage_dir in env.GetLibSourceDirs():
        ...
        if not isdir(storage_dir):
            continue
        handle_lib_builders(storage_dir, env)
...
def handle_lib_builders(storage_dir, env):
...
def ConfigureProjectLibBuilder(env):
    ...
    bit_ly_configure_pio_ldf_url = read_url_json().get('bit_ly_configure_pio_ldf_url', "")
    click.echo(f"LDF: Library Dependency Finder -> {bit_ly_configure_pio_ldf_url}")
    click.echo(
