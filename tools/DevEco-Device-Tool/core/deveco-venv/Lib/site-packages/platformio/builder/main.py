...
env.ConvertToProjectOptions(clivars.keys())
...
env.Replace(
    ...
    CUSTOM_TOOL_PATH=ARGUMENTS.get("PIOCUSTOMTOOL", ''),
    LIBSOURCE_DIRS=[
...
env.CheckCustomCommand()

if env.GetOption("clean"):
    if env.subst("$CLEAN_CMD_CUSTOM"):
        env.PioCustomClean(env.subst("$CLEAN_CMD_CUSTOM"))
    else:
        env.PioClean(env.subst("$BUILD_DIR"), env.subst("$BUILD_OUT_DIR"))
    env.Exit(0)
elif not int(ARGUMENTS.get("PIOVERBOSE", 0)) and not int(ARGUMENTS.get("PIOLISTFILES", 0)):
...
if int(ARGUMENTS.get("PIOLISTFILES", 0)):
    target_req = {}

    for target in BUILD_TARGETS:  # pylint: disable=undefined-variable
        file_list = []

        for src in env.ans[target].sources:
            file_list.append(join(src.dir.get_internal_path(), src.name))

        target_req[target] = file_list

    print(json.dumps(target_req))
    env.Exit(0)
