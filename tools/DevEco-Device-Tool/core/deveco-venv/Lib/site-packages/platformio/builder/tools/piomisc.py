...
from os.path import basename, isdir, isfile, join, realpath, relpath, sep, exists
from tempfile import mkstemp

import hb_adapter # pylint: disable=import-error
from SCons.Action import Action  # pylint: disable=import-error
from SCons.Script import ARGUMENTS  # pylint: disable=import-error
from SCons import Errors  # pylint: disable=import-error
...
class InoToCPPConverter(object):
    ...
    def merge(self, nodes):
        if not nodes:
            raise ValueError(f"Invalid value: {nodes}")
        lines = []
        for node in nodes:
    ...
    def process(self, contents):
        ...
        if not self._gcc_preprocess(contents, out_file):
            raise FileNotFoundError(f"_gcc_preprocess failed,not a file:{out_file}")
...
def _clean_dir(target_dir):
    if not isdir(target_dir):
        return
    clean_rel_path = relpath(target_dir)
...
def CheckCustomCommand(env):
    def check_cmd(target, cmd_line, option_name):
        shell_chars = r'#;`,*?~^()[]{}\$'
        if cmd_line and target in COMMAND_LINE_TARGETS and set(shell_chars) & set(cmd_line):
            click.secho(f'{option_name} can not contain: {shell_chars}', fg='red')
            env.Exit(-1)

    check_table = {
        'buildprog': {'cmd_line': env.subst("$BUILD_CMD_CUSTOM"), 'option_name': 'custom_build_command'},
        'clean': {'cmd_line': env.subst("$CLEAN_CMD_CUSTOM"), 'option_name': 'custom_clean_command'},
        'make': {'cmd_line': env.subst("$MAKE_CMD_CUSTOM"), 'option_name': 'custom_make_command'},
    }

    for target, cmd in check_table.items():
        check_cmd(target, cmd.get('cmd_line', ''), cmd.get('option_name', ''))


def PioClean(env, clean_cache, clean_dir):
    _clean_dir(clean_cache)
    if hasattr(env, 'Cleaner'):
        env.Cleaner()
    else:
...
def PioCustomClean(env, custom_cmd):
    click.secho(custom_cmd, fg="green")
    try:
        sysenv = environ.copy()
        sysenv["PATH"] = str(env["ENV"]["PATH"])
        result = exec_command(custom_cmd, shell=True, env=sysenv, stdout=None)
    except Exception as ex:
        raise Errors.BuildError(errstr=f"Custom clean cmd error") from ex
    env.Exit(result.get("returncode", 1))


def HbClean(env, build_product, out_path):
    def is_ohos_cfg_for(product):
        product_name, product_vendor = product.split('@')
        ohos_config = join(env.subst("$PROJECT_DIR"), 'ohos_config.json')
        try:
            data = json.load(open(ohos_config, 'r'))
            cfg_product = data.get('product', '')
            cfg_product_path = data.get('product_path', '')
            parts = cfg_product_path.split('/')
            if product_name == cfg_product and len(parts) > 1 and parts[-2] == product_vendor:
                return True
        except (json.JSONDecodeError, FileNotFoundError):
            return True
        return False

    if not is_ohos_cfg_for(build_product):
