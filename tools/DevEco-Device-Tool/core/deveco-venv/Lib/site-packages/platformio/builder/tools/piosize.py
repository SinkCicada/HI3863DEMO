...
def _run_tool(cmd, env, tool_args):
    ...
    with os.fdopen(os.open(tmp_file, flags, 0o640), 'w') as fp:
        fp.write("\n".join(tool_args))
...
def _get_symbol_locations(env, elf_path, addrs):
    ...
    if len(addrs) != len(locations):
        raise ValueError(f"The number of items in {addrs} and {locations} not equals")
...
def _get_demangled_names(env, mangled_names):
    ...
    if len(mangled_names) != len(demangled_names):
        raise ValueError(f"The number of items in {mangled_names} and {demangled_names} not equals")
...
def DumpSizeData(_, target, source, env):  # pylint: disable=unused-argument
    ...
    flags = os.O_RDWR | os.O_CREAT | os.O_TRUNC
    with os.fdopen(
            os.open(join(env.subst("$BUILD_DIR"), "sizedata.json"), flags,
                    0o640), 'w') as fp:
