...
def cli(ctx, force, caller, no_ansi):
    ...
    try:
        ...
        setattr(threading.current_thread(), 'stop_event', stop_event)
        ctx.obj['stop_event'] = stop_event
        ...
        if bin_path not in sys_path:
            os.environ['PATH'] = os.pathsep.join([bin_path] + sys_path)
...
def main(argv=None):
    ...
    if argv:
        if not isinstance(argv, list):
            raise TypeError(f"{argv} not a list")
        sys.argv = argv
    try:
        configure()
