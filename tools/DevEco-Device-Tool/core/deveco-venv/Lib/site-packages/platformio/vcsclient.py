...
class VCSClientFactory(object):
    ...
    def newClient(src_dir, remote_url, silent=False):
        ...
        if isinstance(obj, VCSClientBase):
            raise TypeError(f"{obj} not the instance of a class or a subclass for VCSClientBase")
        return obj
...
class VCSClientBase(object):
    ...
    def check_client(self):
        try:
            if not self.command:
                raise ValueError(f"Invalid value: {self.command}")
            if self.silent:
                self.get_cmd_output(["--version"])
            else:
                if not self.run_cmd(["--version"]):
                    raise PlatformioException("run_cmd failed!")
        except (OSError, PlatformioException) as e:
    ...
    def run_cmd(self, args, **kwargs):
        ...
        try:
            ...
            raise PlatformioException("VCS: Could not process command" ) from e
...
class GitClient(VCSClientBase):
    ...
    def export(self):
        ...
        if not self.run_cmd(args):
            raise PlatformioException("run_cmd failed!")
        if is_commit:
            if not self.run_cmd(["reset", "--hard", self.tag]):
                raise PlatformioException("run_cmd failed!")
            return self.run_cmd(
