...
platformio_url = read_url_json().get('platformio_url', '')
...
@click.option(
    "-g", "--global", is_flag=True, help="Manage global DevEco library storage"
)
@click.option(
    "-e",
    "--environment",
...
def cli(ctx, **options):
    ...
    for storage_dir in storage_dirs:
        ...
        with fs.cd(storage_dir):
            handle_storage_config(storage_dir, options, ctx, in_silence)


def handle_storage_config(storage_dir, options, ctx, in_silence):
    config = ProjectConfig.get_instance(
        os.path.join(storage_dir, '.deveco', "deveco.ini")
    )
...
@click.option(
    "--save",
    is_flag=True,
    help="Save installed libraries into the `deveco.ini` dependency list",
)
...
def lib_install(ctx, **kwargs):
    ...
    for storage_dir in storage_dirs:
        ...
        if libraries:
            ...
            handle_install_lib(storage_libdeps, lm, storage_dir, installed_manifests, args)
    ...
    for input_dir in input_dirs:
        config = ProjectConfig.get_instance(os.path.join(input_dir, ".deveco", "deveco.ini"))
        ...
        for env in config.envs():
            ...
            try:
                ...
                lib_deps = []
            handle_lib_config(libraries, lib_deps, installed_manifests, config, env)


def handle_install_lib(storage_libdeps, lm, storage_dir, installed_manifests, args):
    builtin_lib_storages = None
    for library in storage_libdeps[storage_dir]:
        try:
            pkg_dir = lm.install(
                library, silent=args.get('silent'), interactive=args.get('interactive'), force=args.get('force')
            )
            installed_manifests[library] = lm.load_manifest(pkg_dir)
        except exception.LibNotFound as e:
            if builtin_lib_storages is None:
                builtin_lib_storages = get_builtin_libs()
            if not args.get('silent') or not is_builtin_lib(builtin_lib_storages, library):
                click.secho("Warning! lib not found", fg="yellow")


def handle_lib_config(libraries, lib_deps, installed_manifests, config, env):
    for library in libraries:
        if library in lib_deps:
            continue
        manifest = installed_manifests[library]
        try:
            if library.lower() == manifest["name"].lower() & semantic_version.Version(manifest["version"]):
                lib_deps.append("{name}@^{version}".format(**manifest))
            else:
                raise ValueError(f"{library.lower()} not equal {manifest['name'].lower()}")
        except ValueError:
...
def lib_update(ctx, libraries, only_check, dry_run, json_output):
    ...
    for storage_dir in storage_dirs:
        ...
        if only_check and json_output:
            handle_update_lib(_libraries, lm, json_result, storage_dir)
        else:
...
def handle_update_lib(_libraries, lm, json_result, storage_dir):
    result = []
    for library in _libraries:
...
def lib_search(query, json_output, page, noninteractive, **filters):  # pylint: disable=unused-argument
    query_list = []
    if not query:
        query_list = []
    ...
    while True:
        ...
        result = util.get_api_result(
            "/v2/lib/search",
            {"query": " ".join(query_list), "page": int(result["page"]) + 1},
            cache_valid="1d",
        )
...
def lib_stats(json_output):
    ...
    print_tabular_data(result)

    for key in ("lastkeywords", "topkeywords"):
        tabular_data = [
            (
                click.style(name, fg="cyan"),
                f"{platformio_url}/lib/search?query=" + quote("keyword:%s" % name),
            )
