...
class EmbeddedTestProcessor(TestProcessorBase):
    ...
    def run(self):
        ...
        try:
            ...
            click.secho('serial error', fg="red", err=True)
            return False
    ...
    def get_test_port(self):
        ...
        if not {"platform", "board"} & set(self.env_options.keys()):
            raise ValueError(f"{self.env_options.keys()} must contains platform or board")
        ...
        while elapsed < 5 and not port:
            port = handle_check_port(board_hwids)
            if port:
                return port
...
def handle_check_port(board_hwids):
