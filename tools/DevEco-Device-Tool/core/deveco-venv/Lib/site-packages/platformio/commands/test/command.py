...
TestCliArgs = collections.namedtuple('TestCliArgs', [
    'ctx',
    'environment',
    'ignore',
    'filter',
...
@click.command("test", short_help="Unit Testing", hidden=True)
...
def cli(  # pylint: disable=redefined-builtin
    ...
    upload_port = kwargs.get("upload_port")
    test_port = kwargs.get("test_port")
    project_dir = kwargs.get("project_dir")
    project_conf = kwargs.get("project_conf")
    without_building = kwargs.get("without_building")
    without_uploading = kwargs.get("without_uploading")
    without_testing = kwargs.get("without_testing")
    no_reset = kwargs.get("no_reset")
    monitor_rts = kwargs.get("monitor_rts")
    monitor_dtr = kwargs.get("monitor_dtr")
    verbose = kwargs.get("verbose")
    test_cli_args = TestCliArgs(ctx, environment, ignore, filter,
                                upload_port, test_port, project_dir, project_conf,
                                without_building, without_uploading, without_testing,
    ...
    with fs.cd(project_dir):
        ...
        for testname in test_names:
            ...
            test_env(results, env_info, test_cli_args)
...
def test_env(results, env_info, test_cli_args):
    testname = env_info.get('testname')
    config = env_info.get('config')
    default_envs = env_info.get('default_envs')
    environment = test_cli_args.environment

    for envname in config.envs():
        section = "env:%s" % envname
        # filter and ignore patterns
        patterns = dict(filter=list(test_cli_args.filter), ignore=list(test_cli_args.ignore))
        for key in patterns:
        ...
        tp = cls(test_cli_args.ctx, testname, envname,
                 dict(
                     project_config=config, project_dir=test_cli_args.project_dir,
                     upload_port=test_cli_args.upload_port, test_port=test_cli_args.test_port,
                     without_building=test_cli_args.without_building,
                     without_uploading=test_cli_args.without_uploading,
                     without_testing=test_cli_args.without_testing, no_reset=test_cli_args.no_reset,
                     monitor_rts=test_cli_args.monitor_rts, monitor_dtr=test_cli_args.monitor_dtr,
                     verbose=test_cli_args.verbose, silent=not test_cli_args.verbose),
                 )
        result = {"env": envname, "test": testname, "duration": time(), "succeeded": tp.process()}
