...
@click.command("ci", short_help="Continuous Integration", hidden=True)
...
def cli(ctx, **kwargs):
    ...
    keep_build_dir = kwargs.get('keep_build_dir')
...
def _copy_project_conf(build_dir, project_conf):
    ...
    if config.has_section("deveco"):
        config.remove_section("deveco")
    config.save(join(build_dir, ".deveco", "deveco.ini"))
