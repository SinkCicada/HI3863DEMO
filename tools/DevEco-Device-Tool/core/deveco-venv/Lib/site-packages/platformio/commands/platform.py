...
def _get_installed_platform_data(platform, with_boards=True, expose_packages=True):
    ...
    if not manifest:
        raise ValueError(f"Invalid value: {manifest}")
    for key in manifest:
    ...
    for name, opts in p.packages.items():
        ...
        handle_get_platform_data(name, installed_pkgs, item, data)

    return data


def handle_get_platform_data(name, installed_pkgs, item, data):
...
@cli.command("search", short_help="Search for development platform", hidden=True)
...
def platform_search(query, json_output):
    ...
    for platform in pm.get_installed():
        ...
        if query and query.lower() not in search_data.lower():
            continue
        platform_data = _get_installed_platform_data(
            platform["__pkg_dir"], with_boards=False, expose_packages=False
        )
        platform_data["versions"] = [platform_data["version"]]
        platform_data.pop("version", None)
        platform_data.pop("__pkg_dir", None)
        platform_data.pop("docs", None)
...
@cli.command("frameworks", short_help="List supported frameworks, SDKs", hidden=True)
@click.argument("query", required=False)
@click.option("--json-output", is_flag=True)
def platform_frameworks(query, json_output):  # pylint: disable=unused-argument
    frameworks = []
    if json_output:
...
@cli.command("install", short_help="Install new development platform", hidden=True)
...
def platform_install(platforms, **kwargs):
    with_package = kwargs.get("with_package")
    without_package = kwargs.get("without_package")
    skip_default_package = kwargs.get("skip_default_package")
    with_all_packages = kwargs.get("with_all_packages")
...
@cli.command("uninstall", short_help="Uninstall development platform", hidden=True)
@click.argument("platforms", nargs=-1, required=True, metavar="[PLATFORM...]")
...
@cli.command("update", short_help="Update installed development platforms", hidden=True)
